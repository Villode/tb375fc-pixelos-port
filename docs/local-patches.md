...
重建这套包需要重做的改动，来自 2026-09-20 至 09-22 的排障记录。改动当时都带着解释性注释，注释随
源码树一起删了，所以这里只保留记录里确定的事实：**改了哪里、做了什么**。记录里没有的（报错原文、
为什么这样改、属于哪一代）一律标 `未记录`，不猜测。本表是重建线索，不是 patch。
...
- 本地清单：`.repo/local_manifests/pixelos_tb375fc.xml`、`TB375FC.xml`，把
  ...
  固定 `lineage-23.2`。
- 可用基线只有 `lineage-23.2` 一代：PixelOS 官方 org 下没有本设备的 device 树，
  `LosSantosPro` 的三个仓库（device / vendor / -kernel）默认分支均为 `lineage-23.2`。
  更早会话记录里出现过的别的分支口径与公开仓库对不上，不要当依据。
